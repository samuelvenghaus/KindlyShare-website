...
import { submitWidgetFeedbackAction, type WidgetFeedbackFormState } from "@/lib/actions/public-widget-actions";
import { SubmitButton } from "@/components/auth/SubmitButton";

const initialState: WidgetFeedbackFormState = {};

export function WidgetFeedbackForm({ companyToken }: { companyToken: string }) {
  const [state, formAction] = useActionState(submitWidgetFeedbackAction, initialState);
  ...
  if (state.success) {
    return (
      <div className="py-4 text-center">
        ...
        <p className="mt-1 text-sm text-muted">Je bericht is goed ontvangen.</p>
      </div>
    );
  }
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <label className="block">
        <span className="mb-1.5 block text-sm font-medium text-foreground">Je naam (optioneel)</span>
        <input
          name="name"
