...
import { exchangeCodeForTokens } from "@/lib/trustpilot/oauth";
...
export async function GET(request: NextRequest) {
  ...
  const expectedNonce = request.cookies.get("trustpilot_oauth_state")?.value;

  if (!code || !state || !expectedNonce) {
    return NextResponse.redirect(new URL("/kanalen?error=invalid_state", request.url));
  }

  const [nonce, encodedBusinessUnitId] = state.split(".");
  if (nonce !== expectedNonce || !encodedBusinessUnitId) {
    return NextResponse.redirect(new URL("/kanalen?error=invalid_state", request.url));
  }

  const businessUnitId = Buffer.from(encodedBusinessUnitId, "base64url").toString("utf8");
  ...
  try {
    ...
    await prisma.platformConnection.upsert({
      where: {
        companyId_platform_externalAccountId: {
          ...
          platform: "trustpilot",
          externalAccountId: businessUnitId,
        },
      },
      create: {
        companyId: session.companyId,
    ...
    response.cookies.delete("trustpilot_oauth_state");
    return response;
  } catch (err) {
    console.error("Trustpilot OAuth callback mislukt:", err);
